...
class ManipulatorTaskManager(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            ...
            "OUTSIDE_BTN_FRONT, INSIDE_BTN_FRONT, DESTINATION_UNLOAD, "
            "HOME, CANCEL, STATUS, RESET"
        )
    ...
    def _arm_done_cb(self, msg: String) -> None:
        ...
        if self._state in ("OUTSIDE_HOMING", "INSIDE_HOMING", "UNLOAD_HOMING", "HOME_ONLY"):
            if text == "home_done":
                if self._pending_result_after_home:
        ...
        if self._state == "OUTSIDE_PRESSING":
            if text == "button_press_done":
                if self.return_home_after_button:
                    self._start_home("OUTSIDE_HOMING", RESULT_OUTSIDE_BTN_DONE)
                else:
                    self._reset_to_idle()
                    self._publish_result(RESULT_OUTSIDE_BTN_DONE)
            elif self._is_failure(text) or text == "cancelled":
                self._fail_task(f"OUTSIDE_PRESS_FAILED:{text}")
            return

        if self._state == "INSIDE_PRESSING":
            if text == "button_press_done":
                if self.return_home_after_button:
                    self._start_home("INSIDE_HOMING", RESULT_INSIDE_BTN_DONE)
                else:
                    self._reset_to_idle()
                    self._publish_result(RESULT_INSIDE_BTN_DONE)
            elif self._is_failure(text) or text == "cancelled":
                self._fail_task(f"INSIDE_PRESS_FAILED:{text}")
            return
    ...
    def _start_home(self, state: str, result_after_home: str) -> None:
    ...
    def _handle_timeout(self) -> None:
        ...
        if state == "OUTSIDE_PRESSING":
            self._fail_task("OUTSIDE_PRESS_TIMEOUT")
            return

        if state == "INSIDE_PRESSING":
            self._fail_task("INSIDE_PRESS_TIMEOUT")
            return
