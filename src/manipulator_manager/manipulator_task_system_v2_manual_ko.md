...
하역 flag `2` 패킷은 ACK를 기다리지 않는다. ESP32는 flag `2`를 받은 뒤 내부에서 하역 동작을 수행한다. ROS 쪽은 flag `2` 패킷을 한 번 전송하면 완료로 처리하며, 하역 후 home 명령을 추가로 보내지 않는다. 평상시 버튼/홈 동작은 기존과 같이 마지막 필드 `1`을 사용한다.
