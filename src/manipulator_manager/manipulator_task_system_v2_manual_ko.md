...
-> /manipulator_hardware/cmd_pos_flag: 3
-> ESP32 serial: CMD_POS,<seq>,<joint1_deg>,<joint2_deg>,<joint3_deg>,<joint4_deg>,3\r\n
...
하역 flag `3`, `2` 패킷은 ACK를 기다리지 않는다. ESP32는 flag `3`을 받은 뒤 하역 준비 동작을 수행하고, 이어서 flag `2`를 받은 뒤 내부에서 하역 동작을 수행한다. ROS 쪽은 flag `2` 패킷을 전송하면 완료로 처리하며, 하역 후 home 명령을 추가로 보내지 않는다. 평상시 버튼/홈 동작은 기존과 같이 마지막 필드 `1`을 사용한다.
